...
const SOCKET_URL =
  (import.meta.env.VITE_SIGNAL_URL as string) || 'http://localhost:4000';

function useSocketIO(roomId: string, onSignal: (data: any) => void) {
  ...
  return (msg: any) => {
    if (socketRef.current?.connected) {
      socketRef.current.emit('signal', msg);
    }
  };
}
  ...
  const onSignal = useCallback(
    (data: any) => {
      ...
      if (data.type === 'offer') session.handleOffer(data.sdp);
        ...
        if (!controlEnabled) return;
        if (agentRef.current.connected) {
          agentRef.current.sendCtrl(data);
        } else {
          handleReplayControl(data);
        }
      }
    },
    [session, isHost, controlEnabled],
  );

  const sendSignal = useSocketIO(roomId, onSignal);

  const start = async () => {
    const s = createRemoteSession(sendSignal);
    setSession(s);
